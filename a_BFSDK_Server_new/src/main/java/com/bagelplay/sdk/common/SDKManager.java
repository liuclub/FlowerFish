...
public class SDKManager implements OnKeyListener,
		OnStickListener, OnStateListener{

	private String TAG	=	this.getClass().getSimpleName();

	private static SDKManager bfsdkm;
	...
	private SocketStub bvs;

	private SocketManager bfm;

	private SocketThirdManager bftm;

	private SocketThirdStub bfts;
	...
	protected SDKManager(final Activity activity)
	{
		...
		bvs			=	new SocketStub(this);
		bfm			=	new SocketManager(this);
		bftm		=	new SocketThirdManager(this);
		bfts		=	new SocketThirdStub(this);
		...
		{
			bfm.start();
			bvs.sendLoginData(applicationContext.getPackageName(),bftm.getPort(),getID());
		}
		bftm.start();
		if(gun_control)
		{
	...
	public void startVoice(int frequency,int channelConfiguration,int audioEncoding,OnVoiceListener ovl)
	{
		...
		bvs.sendVoiceStart(frequency, channelConfiguration, audioEncoding);
	}
	...
	{
		voice.setOnVoiceListener(ovl);
		bvs.sendVoiceStartByPlayerID(playerID, frequency, channelConfiguration, audioEncoding);
	}

	public void stopVoice(int playerID)
	{
		voice.setOnVoiceListener(null);
		bvs.sendVoiceStopByPlayerID(playerID);
	}
	...
	public void onControllerKeyDown(int playerOrder, int keyCode)
	{
		...
		if(osdkkl != null)
		{
			osdkkl.OnSDKKeyDown(playerOrder, keyCode);
		}
	}
	...
	public void onControllerKeyUp(int playerOrder, int keyCode)
	{
		...
		if(osdkkl != null)
		{
			osdkkl.OnSDKKeyUp(playerOrder, keyCode);
		}
	}
