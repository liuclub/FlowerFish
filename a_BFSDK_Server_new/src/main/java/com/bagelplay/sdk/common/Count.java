...
public class Count {
	...
	private void uplodeLastCount() {
		...
		try {
			...
			String json = Utils.inStreamToStr(is);
	...
	public void doOneTimeCount() {
		...
		String macAddr = Utils.getMacAddressStr(context);
	...
	private void uplodeFirstCount() {
		try {
			HttpP p = new HttpP(WebHost.COUNT_URL + "?first=true");
			p.appendValue("json", jo.toString());
			int res = p.post();
			if (res == 0) {
				String id = p.getResponse();
				putData("id", id);
			}
		} catch (Exception e) {
			e.printStackTrace();
